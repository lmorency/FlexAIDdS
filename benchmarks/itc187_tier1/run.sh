#!/usr/bin/env bash
# run.sh — Execute ITC-187 tier-1 benchmark and validate against baselines
set -euo pipefail
...
PEARSON_R=$(parse_baseline "scoring_power_pearson_r" || echo "0.93")
RMSE=$(parse_baseline "scoring_power_rmse" || echo "1.40")
DOCK_TOP1=$(parse_baseline "docking_power_top1" || echo "0.78")
ENTROPY_RESCUE=$(parse_baseline "entropy_rescue_rate" || echo "0.72")

echo "=== ITC-187 Tier-1 Benchmark ==="
...
if [[ ! -d "${DATA_DIR}" ]] || [[ ! -f "${DATA_DIR}/1a4g.pdb" ]]; then
