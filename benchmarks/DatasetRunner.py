...
Library::
    ...
    runner = DatasetRunner(
        ...
        results_dir="results/benchmark_run",
    )
    report = runner.run_all(tier=1)
    json_path, md_path = report.save("results/benchmark_run/report")

CLI (via ``benchmarks/run.py``)::

    python -m benchmarks.run --dataset casf2016 --tier 1
    python -m benchmarks.run --all --distributed --nodes 4

MPI distributed run::

    mpirun -n 8 python -m benchmarks.run --all --distributed
"""
...
def _flexaidds_version() -> str:
    try:
        ...
        try:
            from python.flexaidds import __version__
            return __version__
        except Exception:
            return "unknown"
...
class DatasetRunner:
    ...
    def __init__(
        self,
        datasets_dir: Union[str, Path] = "benchmarks/datasets",
