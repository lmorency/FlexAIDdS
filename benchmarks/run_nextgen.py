...
"""CLI entry point for the next-generation FlexAIDdS benchmark runner."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m benchmarks.run_nextgen",
        description="FlexAIDdS next-generation clean-room benchmark control plane",
    )
    ...
    parser.add_argument("--datasets-dir", default="benchmarks/datasets", metavar="DIR")
    parser.add_argument("--results-dir", default="results/benchmarks", metavar="DIR")
    parser.add_argument("--data-dir", default=None, metavar="DIR")
    parser.add_argument("--binary", default=None, metavar="PATH")
    parser.add_argument("--temperature", type=float, default=300.0, metavar="K")
    parser.add_argument("--bootstrap", action="store_true")
    parser.add_argument("--n-bootstrap", type=int, default=5000, metavar="N")
    parser.add_argument("--max-job-attempts", type=int, default=2, metavar="N")
    ...
    parser.add_argument("--report-prefix", default=None, metavar="PATH")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.manifest:
        logging.info("Manifest hint: %s", args.manifest)
    if args.executor:
        logging.info("Executor hint: %s", args.executor)
    ...
    prefix = args.report_prefix or str(Path(args.results_dir) / f"report_{timestamp}")
