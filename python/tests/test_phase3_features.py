...
class TestEntropyHeatmapHelpers:
    """Test entropy heatmap computation (no PyMOL needed)."""

    def test_read_pose_coords_filters_hydrogens(self, sample_pdb):
        """Hydrogen atoms should be excluded from coordinate extraction."""
        # We need to import without PyMOL, so mock it
        pymol_mock = types.ModuleType("pymol")
        pymol_mock.cmd = MagicMock()
        with patch.dict(sys.modules, {"pymol": pymol_mock}):
            # Import the helper directly
            from pymol_plugin.entropy_heatmap import _read_pose_coords
            coords = _read_pose_coords(str(sample_pdb))
...
class TestModeAnimationHelpers:
    """Test mode animation coordinate interpolation (no PyMOL needed)."""
